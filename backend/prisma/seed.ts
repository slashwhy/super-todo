...
6. Listen to music or an upbeat playlist`,
  ...
  // Task 9: Grocery shopping
  await prisma.task.create({
    data: {
      title: 'Grocery Shopping for the Week',
      description:
        'Buy vegetables, fruits, milk, bread, and snacks for the family. Check pantry before leaving.',
      isVital: false,
      statusId: statusNotStarted.id,
      priorityId: priorityLow.id,
      categoryId: categoryFamily.id,
  ...
  await prisma.task.create({
    data: {
      ...
      description:
        'Review the authentication module refactor PR. Check for security issues and test coverage.',
      isVital: true,
      statusId: statusInProgress.id,
      priorityId: priorityExtreme.id,
      categoryId: categoryWork.id,
      ownerId: teamMember1.id,
  ...
  // Task 11: Doctor appointment
  await prisma.task.create({
    data: {
      title: 'Annual Health Checkup',
      description:
        'Visit Dr. Smith for annual physical. Bring insurance card and list of medications.',
      isVital: true,
      statusId: statusNotStarted.id,
      priorityId: priorityExtreme.id,
      categoryId: categoryHealth.id,
  ...
  await prisma.task.create({
    data: {
      ...
      description: 'Document the new color tokens and component variants added in sprint 14.',
      isVital: false,
      statusId: statusInProgress.id,
      priorityId: priorityLow.id,
      categoryId: categoryWork.id,
  ...
  // Task 13: Family dinner
  await prisma.task.create({
    data: {
      title: 'Plan Family Dinner for Saturday',
      description:
        'Reserve restaurant or plan home-cooked meal. Check dietary preferences with everyone.',
      isVital: false,
      statusId: statusNotStarted.id,
      priorityId: priorityModerate.id,
      categoryId: categoryFamily.id,
  ...
  // Task 14: Deploy staging
  await prisma.task.create({
    data: {
      title: 'Deploy v2.1 to Staging',
      description:
        'Run the CI pipeline and deploy the latest build to the staging environment. Verify smoke tests pass.',
      isVital: true,
      statusId: statusCompleted.id,
      priorityId: priorityExtreme.id,
      categoryId: categoryWork.id,
      ownerId: teamMember2.id,
      assigneeId: teamMember2.id,
      completedAt: new Date('2023-06-19'),
      createdAt: new Date('2023-06-18'),
    },
  })
  ...
  await prisma.task.create({
    data: {
      ...
      description: 'Complete chapters 8-12. Take notes on habit stacking and environment design.',
      isVital: false,
      statusId: statusInProgress.id,
      priorityId: priorityLow.id,
      categoryId: categoryPersonal.id,
  ...
  const priorityCount = await prisma.taskPriority.count()
