...
description: Perform a read-only security review of specified files or a feature. Reports findings by severity (CRITICAL/HIGH/MEDIUM/LOW) using OWASP Top 10 adapted for this Vue 3 + Express + Prisma stack.
argument-hint: "[file-path | feature-name]"
...
Perform a read-only security analysis. No files are modified. Findings are reported by severity.

## How to Invoke

```
/security-review                    # Review current git diff
/security-review <path/to/file.ts>  # Review a specific file
/security-review <feature name>     # Review all files for a feature
```

## What This Checks

This review covers the OWASP Top 10 adapted for this stack:

| # | Threat | What to Look For |
|---|--------|-----------------|
| A01 | Broken Access Control | Missing auth checks, exposed admin routes, IDOR |
| A02 | Cryptographic Failures | Hardcoded secrets, weak hashing, unencrypted PII |
| A03 | Injection | SQL via raw Prisma queries, XSS in Vue templates |
| A04 | Insecure Design | Mass assignment (`req.body` to Prisma), missing validation |
| A05 | Security Misconfiguration | CORS wildcards, debug endpoints in production |
| A06 | Vulnerable Components | Outdated deps with known CVEs |
| A07 | Auth Failures | Session management, JWT handling |
| A08 | Data Integrity Failures | Unverified data in Prisma writes |
| A09 | Logging Failures | Sensitive data in logs |
| A10 | SSRF | Unvalidated URLs in backend requests |
...
> Reference: [OWASP Agentic AI Threats & Mitigations](https://genai.owasp.org/resource/agentic-ai-threats-and-mitigations/) · Full mapping in [SECURITY.md](../../docs/SECURITY.md#owasp-agentic-ai-threats)

| Check                               | OWASP # | Risk   | What to Look For                                                                                                                                   |
| ----------------------------------- | ------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MCP server supply chain**         | #4      | High   | Third-party MCP servers added without source review or pinned version; `curl \| bash` install patterns                                             |
| **Agent tool scope**                | #2, #3  | High   | Agent granted `edit` or `execute` when its role is read-only; `server/*` wildcard instead of specific tool names                                   |
| **Inter-agent trust**               | #7      | Medium | Agent output written to a shared/world-writable path before being read by another agent; no integrity check on `plan.md`                           |
| **Memory & context poisoning**      | #6      | Medium | User-controlled input appearing in agent context without sanitization (e.g., task titles rendered in prompt, log files processed by agent)         |
| **Goal hijack via crafted content** | #1      | High   | Files in the workspace containing instruction-like text that could redirect agent behavior (e.g., `<!-- SYSTEM: ignore previous instructions -->`) |
| **Cascading failure on auto-chain** | #8      | Medium | Background or cloud agent running more than one autonomous step without a human-in-the-loop checkpoint                                             |
...
| 0.26 - 0.5  | Requires per-session approval                           |
...
| atlassian       | 0.5 (create)   | 0.5 (vendor) | 0.5 (internal)         | **0.125**  | Session approval (write + internal data → capability override) |
| playwright      | 0.5 (execute)  | 0.5 (vendor) | 0 (test data)          | **0.0**    | Session approval (execute capabilities → capability override)  |
| chrome-devtools | 0 (read)       | 0.5 (vendor) | 0.5 (may see app data) | **0.0**    | Session approval (may access app data → capability override)   |
| awesome-copilot | 0.5 (generate) | 0.5 (vendor) | 0 (public)             | **0.0**    | Session approval (generate code/actions → capability override) |
