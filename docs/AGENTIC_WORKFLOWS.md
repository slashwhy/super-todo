...
> AI-powered repository automation that augments — never replaces — the human development pipeline.

**Audience:** All developers | **Prerequisites:** [Custom Agents][custom-agents] · [Security][security]
...
**Traditional CI** (non-agentic): [CI Pipeline](#ci-pipeline) — lint, build, test on every push/PR.

---

## 🎯 What Are Agentic Workflows?

Agentic workflows are a new kind of GitHub Actions workflow. Instead of scripting each step in YAML, you write a **Markdown file** describing what you want. The Copilot engine reads your intent, executes within constrained permissions, and produces **safe outputs** — a controlled write channel that limits what the AI can actually do.
...
```
┌─────────────┐     ┌──────────────────┐     ┌─────────────────┐     ┌──────────────┐
│  Event       │ ──► │  Workflow.md      │ ──► │  Copilot Engine  │ ──► │  Safe Output  │
│  (trigger)   │     │  (intent + rules) │     │  (reasoning)     │     │  (constrained)│
└─────────────┘     └──────────────────┘     └─────────────────┘     └──────────────┘
                           │                          │
                           ▼                          ▼
...
**Key principle:** Workflows have **read-only permissions** on repository content. All writes go through **safe-outputs** — a controlled channel that limits output type, count, and scope.
...
- **Network** is restricted to `defaults` and `node` — no arbitrary internet access
- **Secrets** are managed via `gh aw secrets` — never hardcoded in workflow files
...
- `.github/agents/implement.agent.md` — codebase conventions
```
...
| Workflow             | Imports                                                 |
| -------------------- | ------------------------------------------------------- |
| PR Code Review       | `implement.agent.md` — conventions for review checklist |
| Test Coverage Gaps   | `test-unit.agent.md` — testing patterns and conventions |
| CI Failure Diagnosis | `implement.agent.md` — codebase context for diagnosis   |
...
4. **Enable GitHub Actions** in repo Settings → Actions
...
**Purpose:** Weekly identification of source files without test coverage. Creates **issues with guidance**, not auto-generated tests — juniors write the tests themselves.
...
**Purpose:** Traditional CI — runs lint, build, and tests on every push and PR.
...
Issue opened
  │
  ├──► Auto-triaged with labels + comment (Issue Triage)
  │
  ▼
...
Junior passes @Code-Review-Trainer AutoMCQ (3/3 correct)
  │
  ▼
Junior opens PR → human reviews
  │
  ├──► Human comments '/copilot review' (PR Code Review)
  │
  ▼
CI runs on PR
  │
  ├──► If failure on main: CI Doctor creates diagnostic issue
  │
  ▼
Weekly: test gap issues appear
  │
  └──► Junior picks up [test-gap] issue → writes tests via @Test-Unit
```
...
- Network access is restricted to `defaults` and `node` — no arbitrary HTTP requests
...
- [AI Development Guide][ai-guide] — Feature index and high-level overview
- [Custom Agents][custom-agents] — Agent definitions referenced by workflows
- [AI-Native Training][ai-native-training] — Junior training pipeline and Plan-Act-Verify protocol
- [Security Guide][security] — Security patterns, AI-ism awareness, MCP risk scoring
